...
/// Trait to allow us to use chrono's Timelike
use chrono::Timelike;
...
pub fn fetch_idx(date: &str, run_hour: u8, forecast_hour: u8) -> io::Result<Vec<IdxEntry>> {
    ...
    let resp = client.get(&url).send().map_err(|e| {
        io::Error::new(io::ErrorKind::Other, format!("HTTP error fetching idx: {}", e))
    })?;
    ...
    let text = resp.text().map_err(|e| {
        io::Error::new(io::ErrorKind::Other, format!("Error reading idx body: {}", e))
    })?;
...
pub fn parse_idx(text: &str) -> io::Result<Vec<IdxEntry>> {
    ...
    for line in text.lines() {
        ...
        let forecast = if parts.len() > 5 { parts[5].to_string() } else { String::new() };
...
pub fn find_field_range(
    ...
    Err(io::Error::new(
        ...
        format!(
            ...
            idx_name,
            level,
            entries.iter().map(|e| format!("{}:{}", e.field_name, e.level)).collect::<Vec<_>>().join(", ")
        ),
    ))
}
...
pub fn fetch_grib2_range(
    ...
    let bytes = resp.bytes().map_err(|e| {
        io::Error::new(io::ErrorKind::Other, format!("Error reading body: {}", e))
    })?;
